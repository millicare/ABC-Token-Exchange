...
function Swap(props: any) {
  ...
  async function handleSelect(i: number) {
    ...
    if (side == "from") {
      ...
      console.log('selecting destination token...', temp);
      setTo(temp);
      if (from) {
              ...
              {openSettings ?
                <SwapSettings setOpenSettings={setOpenSettings} slip={[slippage, setSlippage]} speed={[speed, setSpeed]} unlimited={[unlimited, setUnlimited]} /> : <div className="card color-box">
                  ...
                  {fetchloading && false ? <Load loaded={loading} /> :
                    <Fragment>
                      <SwapCard switchHandler={switchHandler} side="from" val={from ? from.value : 0} onChangeBalance={onChangeBalance} setSide={setSide} show={currShow} setShow={setCurrShow} handleValue={handleValue} address={from ? from.address : null} selected={from} from={from} to={to} />
                      <SwapCard side="to" val={from ? from.value : 0} rate={selectedRate} address={to ? to.address : null} onChangeBalance={onChangeBalance} setSide={setSide} show={currShow} setShow={setCurrShow} hanleValue={handleValue} selected={to} from={from} to={to} />
                    </Fragment>}
